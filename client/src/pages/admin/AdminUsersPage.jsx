...
export function AdminUsersPage() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['admin','users', { page, limit, search, role: effectiveRole }],
    queryFn: () => adminUsersApi.list({ page, limit, ...(search?{search}:{}), ...(effectiveRole!=='all'?{role: effectiveRole}:{}) }),
    ...
    <div className="w-full min-w-0 max-w-full space-y-4 overflow-hidden">
      ...
      {isLoading ? (
        ...
        <Card className="flex min-h-[180px] flex-col items-center justify-center border-slate-200 bg-white px-6 py-8 text-center">
          <p className="text-[14px] font-semibold text-slate-900">{search || effectiveRole!=='all' ? 'No matching users' : 'No users yet'}</p>
        ...
        <>
            ...
            <div className="max-h-[65vh] min-h-[360px] overflow-auto">
              <table className="w-full text-xs">
                ...
                <tbody className="divide-y divide-slate-100">
                    {items.map(u=> (
                    <tr key={u.id} className="h-12 hover:bg-slate-50">
                      ...
                      <td className="px-2 py-2">
                        <span className={`inline-flex rounded px-1.5 py-0.5 text-xs font-medium capitalize ${ROLE_BADGE[u.role] || ROLE_BADGE.user}`}>{u.role === 'user' ? 'User' : u.role}</span>
                      </td>
                      <td className="px-2 py-2">
                        {u.emailVerified ? (
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 text-xs">
            <span className="text-slate-600">Showing {start}–{end} of {total} users</span>
